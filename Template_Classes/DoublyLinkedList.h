...
void LinkedList<T>::printList() const
{
    ...
    while (temp != nullptr)
    {
        // REMOVED: cout << temp->getData();
        // Instead, we'll just indicate there's data without trying to print it
        // If T has a display() method, call it instead
        cout << "[Data Object]";
...
ostream &operator<<(ostream &out, const LinkedList<T> &list)
{
    ...
    while (temp != nullptr)
    {
        // Don't try to print the data directly if T doesn't have operator<<
        // Instead, we can indicate there's data
        out << "[Data Object]";
        if (temp->getNext() != nullptr)
            out << " <-> ";
        temp = temp->getNext();
